...
- 用户说“请处理新增游戏后续工作”时：按 §7B 完成组装、验收、测试、构建和本地预览；
  除非用户明确要求，不推送、不发布、不覆盖外部 AI 交付的玩法代码。
- 外部网页 AI 的职责仅是产出完整 `games/<slug>/` 文件；它们无法运行 node/浏览器时，
  由本代理负责机器验收、逻辑审查、门户物料和修复。外发时优先贴 `docs/GAME-SPEC.md` 正文，
  不要求对方理解 GitHub Actions、gh-pages、CNAME 等部署细节。
