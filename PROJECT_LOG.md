...
- **Mole 莓园打地鼠 / Berry Bash（新增，2026-09-23）**：**阳光莓园里的经典街机打地鼠**，按 `docs/plans/mole-prd.md` 本地端到端（模式 A）落地（拍板 **B 纯经典计分** / **B 三档难度无尽 轻松·普通·疯狂** / **A 首发含每日固定种子题面**）。
  ...
  - **视觉回归 5 轮（用户逐轮目视验收驱动）**：
    ...
    5. *砸下去没有动画感* → 旧实现的硬伤不是时长而是**结构**：`.18s` 且首帧直接就是抬起态，**没有蓄力段**，砸落被压在 ~80ms 内。改为 `240ms` 五帧四拍（静止 -14° → 蓄力 -46° → 落槌 +14° → 回弹 -24° → 归位），并**把定位与姿态拆成两层**（`.hammer` 只管 fixed 定位+显隐，`.hammer-rig` 只管旋转基点 `transform-origin: 60px 66px`）—— 旋转动画若挂在带负 margin 的外壳上，百分比 `translate` 会与 margin 叠加导致首帧错位。另补三样打击反馈：**落点冲击波** `.hole.is-impact::after`（与 `.is-hit` 同位、更短收，先炸后陷两拍错开）、**屏幕微震** `.garden.is-shake`（只震舞台不震 body，炸弹用 300ms 加强版）、**挥击拖影** `.hammer.is-swing::before`（伪元素残影，零额外 DOM）。实算验证：蓄力 38ms + 落槌 82ms + 回弹 120ms，摆幅 60°，无抢跑。
  ...
  - **最终门禁**：单测 **192/192**（engine / game / storage / i18n / assembly / markup / layout 七文件）、`check-game mole` **19 pass / 0 fail(T1) / 0 warn(T2)**、**3 视口（1280×800 / 390×844 / 834×1112）冒烟各 60/60**（含 10 项木槌链路 + 8 项打击反馈：挥锤动画/冲击波/微震/炸弹加强版）、`npm run build` 通过且 `dist/mole/` 已同步新代码。
