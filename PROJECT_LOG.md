...
## 2026-09-25 · 割草！（mow）幸存者割草 bullet-heaven 端到端上线（模式 A）
- **玩法核心**：花田幸存割草，对标 Vampire Survivors / Brotato / 弹壳特攻队。自动攻击、走位躲怪、升级三选一，8 分钟一局。按 `docs/plans/mow-prd.md` 落地（拍板 1A 花园割草机题材 / 2A 标准一局 + 无尽深渊 / 3A 武器×被动进化合成）。
- **模式编排（拒绝三件套）**：标准一局 450s 三阶段（检查点 120/240/360 全屏清场 + 回满血），7:30 迎战园丁巨人 Boss（hp 1500、缓慢追踪玩家 `targetY=clamp(p.y-240,130,560)`，击杀三星通关）；无尽深渊通关解锁；圆锯兔通关解锁。评价体系 = 三星星级 + 击杀/连击/得分，非无尽冲分。
- **构筑池**：4 机台（割草机/花洒/瓢虫/圆锯兔）+ 8 武器（blade-ring/pollen-spray/bouncy-pod 初始，thorn-vine 30s / sunbeam 60s / ladybug-strike 90s / bee-swarm 120s / dandelion-bomb 150s 渐次解锁）+ 8 被动 + 6 进化超武（武器满 8 级 + 对应被动 ≥1 级 + 捡精英宝箱）；6 敌（毛毛虫→甲虫→野蜂→毒蘑菇死爆溅射→荆棘团弹跳→巨型甲虫精英必掉进化宝箱）。
- **算法与无死局三重保证**：怪物出生环带（距玩家 ≥420px + 0.4s 淡入无敌，12 次尝试不满足即跳过）；每 2s 逃生走廊校验（8 方向全围→免费 emergency burst 330px + 1.5s 无敌，Boss 战不触发）；Boss 登场前全屏清场。`mulberry32` 确定性种子；固定步长 `stepFrame` 1/120s（dt 钳 0~1/20s）+ 离散意图 `applyIntent`（无效返回 null、终局 no-op）；1200 步随机游走断言不抛错/不卡死/不变式不破，同种子同轨迹。
- **数值调优史（三轮）**：首轮 90s 仅 85 杀过疏 → spawnInterval 0.30-0.17k + 批量出生 batch=1+floor(t/120)（上限4）+ hp 缩放 1+(t/480)*1.2；二轮 4454 杀/5260 分三星但割草槽涨太快与得分虚高 → mow 增长降档 `min(4, 0.7+xp*0.12+combo*0.012)` + KILL_BASE 与连击倍率降档；三轮 Boss 战 480s 才 99% 血暴露击杀窗口偏紧 → **Boss hp 2000→1500** + y 由钉死 170 改缓慢追踪 → 标准局 484s 击杀 Boss 三星冒烟确认。
- **模块分层**：engine(DOM-free 规则权威) / game(DOM-free 控制器，固定步长累加器 max 30 步防螺旋，终局 finishRun 归档一次) / score(SCORE_CAP 999999 钳制、killScore 连击倍率最高 ×2.5、starsFor/bestOf) / storage(`doin.mow.v1`，applyResult 返回 `{state,improved,newUnlock}`) / i18n(约 140 键双表 `doin.lang`) / audio(WebAudio 程序化合成) / render(Canvas 糖果花园软胶风，预渲染辉光精灵 + 静态背景缓存，高亮一律径向柔光 createRadialGradient 渐隐禁描边圈 + 呼吸脉动 + reduced-motion 降级) / main(装配 + rAF)。
- **美学与红线**：双翼糖果机台——木牌顶栏收纳返回/音效/语言/帮助（拒绝四角散落），左翼园艺工坊（武器/被动/进化配方 + 出击/图鉴），中央花田舞台（木质画框 + 景深光晕 + 环境渐变），右翼战况牌匾（倒计时/生命/割草槽/连击/击杀/等级/历史最佳/图鉴进度）；实体按键打击质感；移动端掌机操作台（摇杆 + 花粉爆发/暂停）+ 底部 `max(68px, calc(16px + env(safe-area-inset-bottom)))` 广告安全避让 + `touch-action:none`；`prefers-reduced-motion` 全量降级；零外链零 CDN、相对路径 + ?v=dev。
