// filepath: games/sokoban/js/storage.mjs
// 唯一本地持久化封装。所有读写都在 try/catch 内；损坏/缺失/不可用时回退内存默认值。
// 存档 key：doin.sokoban.v1（语言偏好是全站共享的 doin.lang，不放在这里）

import { LEVELS } from "./levels.mjs";
import { PERFECT } from "./score.mjs";

export const STORAGE_KEY = "doin.sokoban.v1";
const VERSION = 1;
...
function normalizeEntry(raw) {
  ...
  const pushes = toInt(raw.bestPushes, 0, 0, 100000);
...
export function normalize(raw) {
  ...
  const unlocked = toInt(raw.unlocked, 1, 1, LEVELS.length);
  const current = toInt(raw.current, 0, 0, LEVELS.length - 1);
  const levels = {};
  ...
  for (let i = 0; i < LEVELS.length; i++) {
    if (levels[LEVELS[i].id]?.cleared) derived = Math.max(derived, Math.min(LEVELS.length, i + 2));
  }
  derived = Math.min(LEVELS.length, Math.max(1, derived));
...
export function recordResult(levelId, { score, pushes, timeMs, stars }) {
  ...
  const safePushes = toInt(pushes, 0, 0, 100000);
  ...
  data.levels[levelId] = {
    ...
    bestPushes: prev?.cleared ? Math.min(safePushes, prev.bestPushes) : safePushes,
    bestTimeMs: prev?.cleared ? Math.min(safeTime, prev.bestTimeMs) : safeTime,
  };

  const index = LEVELS.findIndex((level) => level.id === levelId);
  if (index >= 0) data.unlocked = Math.min(LEVELS.length, Math.max(data.unlocked, index + 2));
