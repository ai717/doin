...
test("renderer keeps the required keyed SVG layers and focusable controls", async () => {
  ...
  assert.match(main, /import \{ CHAPTERS, LEVELS, levelById \}/);
  assert.match(main, /section\.dataset\.theme = chapter\.theme/);
  assert.match(main, /renderer\.setTheme\(level\.theme/);
  ...
  assert.match(main, /function showLevelInfo\(\)/);
  assert.match(main, /const inferredUnlockedLevel = Math\.max\(1, \.\.\.completedIds/);
  assert.match(main, /const selectedLevel = levelById\(Number\(levelId\)\)/);
