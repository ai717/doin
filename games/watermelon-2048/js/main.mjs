...
// 摘瓜分流：松手时命中 2048 大西瓜 → 摘瓜，否则在落点丢弃。
...
import { advanceFrame, createGame, dispatch, snapshot } from "./game.mjs?v=dev";
import { createRenderer } from "./render.mjs?v=dev";
import { createAudio } from "./audio.mjs?v=dev";
import { mountUI } from "./ui.mjs?v=dev";
import * as storage from "./storage.mjs?v=dev";
import { format, htmlLang, loadLocale, saveLocale, strings } from "./i18n.mjs?v=dev";
...
function bestFor(m) {
  if (m === "daily") {
    return persist.daily.date === todayKey()
      ? { best: persist.daily.best, bestTime: persist.daily.bestTime }
      : { best: 0, bestTime: 0 };
  }
  return { best: persist.endless.best, bestTime: persist.endless.bestTime };
}
  ...
  for (const event of events) {
    switch (event.type) {
      ...
      case "merge": {
        ...
        renderer.ripple(event.x, event.y, event.level);
        renderer.popText(event.x, event.y, event.value, event.chainIndex);
        renderer.setShake(Math.min(9, 2 + event.level * 0.7));
        if (event.chainIndex > 0) {
          audio.chain(event.chainIndex);
          renderer.burst(event.x, event.y, event.level);
        }
      ...
      case "harvest":
        audio.harvest();
        renderer.harvestFx(event.x, event.y, MAX_LEVEL);
      ...
      case "roundEnd": {
        ...
        const tip =
          event.mode === "daily"
            ...
            : t.resultTipEndless;
        ui.showResult(game.state, { best: info.best, bestTime: info.bestTime, isNewBest, tip });
        break;
      }
      case "deny":
        if (event.reason === "blocked" || event.reason === "full") ui.toast(t.toastBlocked);
        else if (event.reason === "noHarvest") ui.toast(t.toastNoHarvest);
...
// —— 指针：移动瞄准，松手时命中 2048 大西瓜则摘瓜，否则在落点丢弃 ——
let aiming = false;
...
canvas.addEventListener("pointerup", (e) => {
  ...
  const p = renderer.toWorld(e.clientX, e.clientY);
  aiming = false;
  if (harvestTargetAt(game.state, p.x, p.y)) {
    send({ type: "harvest", x: p.x, y: p.y });
  } else {
    send({ type: "aim", x: p.x });
