...
## 3.2 · 桌面端 UI 美学与游戏化设计（硬性要求）

1. **桌面端双栏沉浸舞台**：移动端（≤768px）紧凑单列自适应；桌面端（≥900px）必须设计舒展的双栏或宽屏沉浸舞台（主棋盘/Canvas 居中或偏左，侧边集成关卡匾额、多维计分牌、操作台与工具栏）。
2. **主题氛围与视觉质感**：依据游戏题材定制色彩与微质感（环境渐变光晕、微阴影、对角装饰），杜绝大片苍白背景与老旧原生按钮。
3. **游戏化 HUD 与反馈动效**：HUD 徽章化/胶囊化，数值变化附带跳动动效（bump）；按钮支持立体微浮起与按下位移；通关结算必须具备胜利用色与星级/徽章成就感仪式动效。
